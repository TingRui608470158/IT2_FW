...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  int16_t ep_addR=0x100; //起始分配地址: 这里是大坑 casojie
  int8_t ep_addr_size=64;
...
#else
		HAL_PCDEx_PMAConfig(pdev->pData , HID_EPIN_ADDR , PCD_SNG_BUF, 0x100);  // HID IN
		HAL_PCDEx_PMAConfig(pdev->pData , CDC_IN_EP ,			PCD_SNG_BUF, 0x140); // CDC IN
		HAL_PCDEx_PMAConfig(pdev->pData , CDC_CMD_EP , 		PCD_SNG_BUF, 0x150); // CDC CMD
		HAL_PCDEx_PMAConfig(pdev->pData , CDC_OUT_EP , 		PCD_SNG_BUF, 0x190); // CDC OUT

//		HAL_PCDEx_PMAConfig(pdev->pData , CDC_IN_EP2 ,			PCD_SNG_BUF, 0x1d0); // CDC IN
//		HAL_PCDEx_PMAConfig(pdev->pData , CDC_CMD_EP2 , 		PCD_SNG_BUF, 0x1e0); // CDC CMD
//		HAL_PCDEx_PMAConfig(pdev->pData , CDC_OUT_EP2 , 		PCD_SNG_BUF, 0x220); // CDC OUT
#endif
...
//      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , CDC_CMD_EP , PCD_SNG_BUF, ep_addR);
...
//      /* USER CODE END EndPoint_Configuration_CDC */
//
//      HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , HID_EPIN_ADDR , PCD_SNG_BUF, 0x150); ep_addR+=ep_addr_size;

#if 0	// only for otg
    HAL_PCDEx_SetRxFiFo(&hpcd_USB_FS, 0x80);
      HAL_PCDEx_SetTxFiFo(&hpcd_USB_FS, 0, 0x40);
      HAL_PCDEx_SetTxFiFo(&hpcd_USB_FS, 1, 0x20);
      HAL_PCDEx_SetTxFiFo(&hpcd_USB_FS, 2, 0x20);
      HAL_PCDEx_SetTxFiFo(&hpcd_USB_FS, 3, 0x40);
    //  HAL_PCDEx_SetTxFiFo(&hpcd_USB_FS, 4, 0x20);
    //  HAL_PCDEx_SetTxFiFo(&hpcd_USB_FS, 5, 0x20);
#endif
    /* USER CODE END EndPoint_Configuration_HID */
