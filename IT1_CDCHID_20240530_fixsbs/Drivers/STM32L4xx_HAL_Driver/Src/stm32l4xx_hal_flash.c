...
uint32_t Flash_Write_Data(uint32_t StartPageAddress, uint64_t *Data, uint16_t numberofdoublewords, uint8_t flashTypeProgram)
{
	...
	{
		if (flashTypeProgram == FLASH_TYPEPROGRAM_DOUBLEWORD ||
				flashTypeProgram == FLASH_TYPEPROGRAM_FAST ||
				flashTypeProgram == FLASH_TYPEPROGRAM_FAST_AND_LAST)
 ...
 *
 *  StartPageAddress: the Start address of the page, from where you want to start reading the data
