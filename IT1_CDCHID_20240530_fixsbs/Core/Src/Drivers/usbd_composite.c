...
uint8_t cdcReceiveBuffer[64] = {0};
uint32_t cdcReceiveBufferLen = 0;
...
USBD_ClassTypeDef  USBD_COMPOSITE =
{
  ...
  NULL,
  NULL,
  USBD_Composite_GetFSCfgDesc,		// full - speed
  NULL,
...
#define SIZEOF_INTERFACE_DESCRIPTOR	9
#define SIZEOF_ENDPOINT_DESCRIPTOR	7

#define EP_DESC_ATTR_TYPE_BULK	0x02
#define EP_DESC_ATTR_TYPE_INT	0x03
#define HID0_REPORT_INTERFACE	7
...
__ALIGN_BEGIN uint8_t USBD_Composite_CfgFSDesc[USBD_COMPOSITE_DESC_SIZE]  __ALIGN_END =
{
		/* 配置描述符 */
		  ...
		  USBD_IAD_DESC_SIZE,                        // bLength IAD描述符大小
		  USBD_IAD_DESCRIPTOR_TYPE,                  // bDescriptorType IAD描述符类型
			USBD_CDC_CMD_INTERFACE,                    // bFirstInterface 接口描述符是在总的配置描述符中的第几个从0开始数
		  ...
		  0x01,                                      // bInterfaceProtocol 设备符中的bDeviceProtocol
			...
			/* 9+9+7=25 */
		  /********************  CDC0 interface ********************/
		  /************** Descriptor of CDC0 interface ****************/
		  /* 09 */
			SIZEOF_INTERFACE_DESCRIPTOR,                   /*bLength: Interface Descriptor size*/
		  USB_DESC_TYPE_INTERFACE,/*bDescriptorType: Interface descriptor type*/
			USBD_CDC_CMD_INTERFACE,     /*bInterfaceNumber: Number of Interface 接口编号 0 */
      ...
      0x01,                              // bInterfaceProtocol: Common AT commands
			...
			USBD_CDC_DATA_INTERFACE,                // bDataInterface: 0
			...
			USBD_CDC_CMD_INTERFACE,                // bMasterInterface -- the controlling intf for the union
			USBD_CDC_DATA_INTERFACE,                // bSlaveInterface -- the controlled intf for the union
			...
			USBD_CDC_DATA_INTERFACE,                // bInterfaceNumber
			0x00,                               // bAlternateSetting: Alternate setting
			...
			/* 9+9+7=25 */
			/********************  CDC1 interface ********************/
			/************** Descriptor of CDC1 interface ****************/
			/* 09 */
			SIZEOF_INTERFACE_DESCRIPTOR,                   /*bLength: Interface Descriptor size*/
			USB_DESC_TYPE_INTERFACE,/*bDescriptorType: Interface descriptor type*/
			USBD_CDC_CMD_INTERFACE2,     /*bInterfaceNumber: Number of Interface 接口编号 0 */
			0x00,                              // bAlternateSetting: Alternate setting
			0x01,                              // bNumEndpoints: Three endpoints used
			0x02,                              // bInterfaceClass: Communication Interface Class
			0x02,                              // bInterfaceSubClass: Abstract Control Model
			0x01,                              // bInterfaceProtocol: Common AT commands
			INTF_STRING_INDEX + 1,                      /*iInterface: Index of string descriptor*/
			...
			0x01,                                // bDescriptorSubtype: Call Management Func Desc
			0x00,                                // bmCapabilities: D0+D1
			USBD_CDC_DATA_INTERFACE2,                // bDataInterface: 0
			...
			0x24,                               // bDescriptorType: CS_INTERFACE
			0x06,                                // bDescriptorSubtype: Union Functional Desc
			USBD_CDC_CMD_INTERFACE2,                // bMasterInterface -- the controlling intf for the union
			USBD_CDC_DATA_INTERFACE2,                // bSlaveInterface -- the controlled intf for the union
			...
			0x09,                                // bLength: Interface Descriptor size
			USB_DESC_TYPE_INTERFACE,                // bDescriptorType: Interface
			USBD_CDC_DATA_INTERFACE2,                // bInterfaceNumber
			0x00,                               // bAlternateSetting: Alternate setting
			0x02,                               // bNumEndpoints: Three endpoints used
			0x0A,                               // bInterfaceClass: Data Interface Class
			0x00,                               // bInterfaceSubClass:
			...
			SIZEOF_ENDPOINT_DESCRIPTOR,         // bLength: Endpoint Descriptor size
			USB_DESC_TYPE_ENDPOINT,                    // bDescriptorType: Endpoint
			CDC_OUT_EP2,                    // bEndpointAddress: (OUT3)
			EP_DESC_ATTR_TYPE_BULK,                // bmAttributes: Bulk
			0x40, 0x00,                         // wMaxPacketSize, 64 bytes
			0xFF,                                 // bInterval: ignored for Bulk transfer
			...
			SIZEOF_ENDPOINT_DESCRIPTOR,         // bLength: Endpoint Descriptor size
			USB_DESC_TYPE_ENDPOINT,                    // bDescriptorType: Endpoint
			CDC_IN_EP2,                        // bEndpointAddress: (IN3)
			EP_DESC_ATTR_TYPE_BULK,                // bmAttributes: Bulk
			0x40, 0x00,                         // wMaxPacketSize, 64 bytes
			0xFF,                                // bInterval: ignored for bulk transfer
		  ...
		  HID_EPIN_ADDR,          /*bEndpointAddress: Endpoint Address (IN)*/
			EP_DESC_ATTR_TYPE_INT,                   /*bmAttributes: Interrupt endpoint*/
			0x40, 0x00,                         // wMaxPacketSize, 64 bytes
			15                                  // bInterval, ms

//			SIZEOF_ENDPOINT_DESCRIPTOR,         // bLength
//			USB_DESC_TYPE_ENDPOINT,                 // bDescriptorType
//			HID_EPOUT_ADDR,                    // bEndpointAddress; bit7=1 for IN, bits 3-0=1 for ep1
...
static uint8_t  USBD_Composite_DataIn (USBD_HandleTypeDef *pdev,
                              uint8_t epnum)
{
//	DPRINTF("\r\n%sUSBD_Composite_DataIn epnum=%x, pCDCData=%p, pHIDData=%p %s", COLOR_CYAN, epnum, pCDCData, pHIDData, COLOR_WHITE);
  switch(epnum)
  {
      case CDC_INDATA_NUM:
         pdev->pUserData =  &USBD_CDC_Interface_fops_FS;
...
uint8_t  USBD_Composite_DataOut (USBD_HandleTypeDef *pdev,
                               uint8_t epnum)
{
//	DPRINTF("\r\n%sUSBD_Composite_DataIn epnum=%x %s", COLOR_CYAN, epnum, COLOR_WHITE);
  switch(epnum)
  {
